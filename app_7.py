...
def main():
    ...
    if (data_file_1 is not None):
        ...
        if graph_type == 'Barras':
            ax[0].bar(bank_raw_target_perc.index, bank_raw_target_perc['proportion'])
            ax[0].set_xticks(bank_raw_target_perc.index)
            ax[0].set_xticklabels(bank_raw_target_perc.index, rotation=45)
            ax[0].set_xlabel('Categorias')
            ax[0].set_ylabel('Proporção')
            ax[0].set_title('Dados brutos', fontweight="bold")

            ax[1].bar(bank_target_perc.index, bank_target_perc['proportion'])
            ax[1].set_xticks(bank_target_perc.index)
            ax[1].set_xticklabels(bank_target_perc.index, rotation=45)
            ax[1].set_xlabel('Categorias')
            ax[1].set_ylabel('Proporção')
            ax[1].set_title('Dados filtrados', fontweight="bold")
        else:
            ax[0].pie(bank_raw_target_perc['proportion'], labels=bank_raw_target_perc.index, autopct='%.2f%%')
            ax[0].set_title('Dados brutos', fontweight="bold")

            ax[1].pie(bank_target_perc['proportion'], labels=bank_target_perc.index, autopct='%.2f%%')
            ax[1].set_title('Dados filtrados', fontweight="bold")
